...
void QuadMesh::PreDraw(const BlendMode& blendMode) {
    commandList_->SetGraphicsRootSignature(modelConfig_->rootSignature->GetRootSignature(0));
    commandList_->SetPipelineState(MyEngine::GetPSO()->GetGraphicsPipelineState(blendMode,kCullModeBack).Get());//PSOを設定
