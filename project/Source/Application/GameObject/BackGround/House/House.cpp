...
House::House()
{
    ...
    colliders_[kHouse_wall_back]->SettingAABB({ .min = {-6.0f,0.0f,-0.25f},.max = {6.0f,12.0f,0.25f} }, ModelManager::GetModel(ModelManager::HOUSE_WALL_BACK));
    colliders_[kHouse_door]->SettingSphere(3.0f,nullptr);

    colliders_[kHouse_wall_back]->pos_.worldTransform_.Parent(floorPos_.worldTransform_);
    ...
    doorPos_.worldTransform_.Parent(colliders_[kHouse_wall_back]->pos_.worldTransform_);
...
void House::Update()
{
    ...
    if (isWallBrake_) {
        ...
        if (isDoorOpen_) {
            ...
            doorPos_.worldTransform_.rotate_.y = colliders_[kHouse_door]->pos_.worldTransform_.rotate_.y;
        }
    }
