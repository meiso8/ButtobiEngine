...
void CollisionManager::CheckCollisionPair(Collider *colliderA, Collider *colliderB) {
	Capsule capsuleA = {
		.segment = {
			.origin = colliderA->GetWorldPosition(),
			.diff = colliderA->GetVelocity() * deltaTime
		},
	...
	Capsule capsuleB = {
		.segment = {
			.origin = colliderB->GetWorldPosition(),
			.diff = colliderB->GetVelocity() * deltaTime
		},
		.radius = colliderB->GetRadius()
	};

	// 衝突判定
	if (IsCollision(capsuleA, capsuleB)) {
